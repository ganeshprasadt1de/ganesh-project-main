...
from discovery_protocol import client_discover_servers
...
class PongClient:
    def __init__(self, player: int):
        ...
        self.log = PONG_CLIENT_LOGGER
        self.last_update_time: float = 0
        self.last_seq: int = -1

    def _find_server(self) -> Optional[Tuple[str, int]]:
        self.log.info(f"[Player {self.player}] Scanning for servers...")
        while True:
            if pygame.get_init():
            ...
            if found:
                sid, sip = found[0]
                self.log.info(f"Found server {sid[:8]} at {sip}")
    ...
    def _reconnect(self, screen=None):
        ...
        while self.server_addr is None and self.running:
            if screen and self.font:
                screen.fill(BLACK)
                text = self.font.render("Reconnecting...", True, WHITE)
                screen.blit(text, (SCREEN_WIDTH // 2 - 130, SCREEN_HEIGHT // 2))
                pygame.display.flip()
    ...
    def _announce(self):
        if self.server_addr:
            ...
            self.log.debug("Announced to server")
    ...
    def _wait_for_room_ready(self, screen=None) -> bool:
        self.room_ready = False
        last_announce = 0.0
        self.log.info("Waiting for opponent...")
        ...
        while self.running and not self.room_ready:
            ...
            if msg_type == MSG_ROOM_READY:
                ...
                self.last_update_time = time.time()
                self.log.info("Room is ready — game starting!")
                return True
    ...
    def draw(self, screen):
        ...
        pygame.draw.rect(
            screen, WHITE,
            (offset, self.state["p1_y"] - paddle_h // 2, paddle_w, paddle_h),
        )
        pygame.draw.rect(
            screen, WHITE,
            (self.state["width"] - offset - paddle_w, self.state["p2_y"] - paddle_h // 2, paddle_w, paddle_h),
        )
        pygame.draw.rect(
            screen, WHITE,
            (self.state["ball_x"] - ball_size // 2, self.state["ball_y"] - ball_size // 2, ball_size, ball_size),
        )
    ...
    def game_loop(self, screen=None, clock=None):
        ...
        while self.running:
            if time.time() - self.last_update_time > CLIENT_SERVER_TIMEOUT:
    ...
    def _handle_game_over(self):
        self.log.info("Game Over — closing client.")
        self.game_over = True
        self.running = False
    ...
    def _shutdown(self):
        ...
        if pygame.get_init():
            pygame.quit()
        self.log.info("Client shut down.")
