...
# Election helper (alternate style)
...
MSG_DISCOVER_REQUEST  = "DISCOVER_REQUEST"   # some code used 'REQUEST'
MSG_DISCOVER_RESPONSE = "DISCOVER_RESPONSE"
MSG_DISCOVER_REPLY    = MSG_DISCOVER_RESPONSE

# ----------------- socket helpers -----------------
def make_udp_socket(bind_ip: str = "", bind_port: Optional[int] = 0, broadcast: bool = False) -> socket.socket:
    """
    ...
    try:
        ...
        pass
    # try reuseport where supported (not available on some platforms)
    try:
...
def _json_dumps_bytes(obj: Any) -> bytes:
    ...
    try:
        ...
        def _fallback(o):
            return str(o)
        return json.dumps(obj, separators=(",", ":"), ensure_ascii=False, default=_fallback).encode("utf-8")

def send_message(sock: socket.socket, addr: Tuple[str, int], payload: Dict[str, Any], clock: Optional[Any] = None) -> None:
    """
    Wrap `payload` into an envelope { "payload": payload, "clock": <ts> } and send via UDP.
    ...
    # best-effort send; let caller handle exceptions if they want
    sock.sendto(data, addr)

def recv_message(sock: socket.socket, clock: Optional[Any] = None, bufsize: int = 65535) -> Tuple[Dict[str, Any], Tuple[str, int]]:
    """
    Receive a UDP datagram and parse it.
    Returns (payload_dict_or_value, (addr_ip, addr_port)).
    If the incoming datagram is the envelope we expect, returns envelope['payload'].
    If JSON parse fails, returns a fallback payload {'raw': <decoded_text>}.
    If clock is provided and the message contained a 'clock' field, calls clock.update_on_recv(ts)
    (if such method exists) to merge logical clocks.
    """
    ...
    if text is not None:
        try:
            ...
            # not valid JSON — return raw text so higher layers can decide
            payload = {"raw": text}
        else:
            if isinstance(obj, dict) and "payload" in obj:
                ...
                # message was plain JSON, treat it as payload directly
                payload = obj
    else:
        payload = {"raw_bytes": data}

    # update lamport clock if available and timestamp present
    if clock is not None and ts is not None:
        try:
            if hasattr(clock, "update_on_recv"):
                clock.update_on_recv(ts)
            elif hasattr(clock, "tick"):
                # best-effort: if no update_on_recv, call tick() to advance local clock
                clock.tick()
        except Exception:
            # don't let clock errors crash message handling
            pass
